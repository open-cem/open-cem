...
with ui.tab_panels(tabs, value="config").classes("w-full"):
    ...
    with ui.tab_panel("config"):
        ...
        with device_management:
            ...
            ui.button(
                "Load EID for config",
                on_click=lambda: GUI_functions.load_local_EIDs(
                    LocalEID_container, param_container
                ),
            )
            ...
            with download_EID:
                ui.label("Download EID if not locally stored").classes(
            ...
            ui.button(
                "Get Params", on_click=lambda: GUI_functions.getParams(param_container)
            )
            ui.label("EID parameter:").classes("text-sm text-gray-600 mb-2")
            param_container = ui.column().classes("w-full")
            ui.button(
                "Add Device", on_click=lambda: GUI_functions.addDevice(param_container)
            )

            ui.button(
                "get Configuration",
                on_click=lambda: GUI_functions.dynamic_pagination(
                    device_management, config_container
                ),
            )
    ...
    with ui.tab_panel("QRcode"):
        with ui.row().classes("gap-4"):
            ...
            ui.button(
                ...
                on_click=lambda: GUI_functions.yaml_workflow(qr_code_config_container),
    ...
    with ui.tab_panel("control"):
        ...
        with ui.card().classes("w-full"):
            with ui.row().classes("gap-2"):
                ui.button(
                    "start OpenCEM",
                    on_click=lambda: GUI_functions.start_OpenCEM(
                        latest_value_container
                    ),
                ).props("color=positive")
                ui.button("stop OpenCEM", on_click=GUI_functions.stop_OpenCEM).props(
                    "color=negative"
                )
    ...
    with ui.tab_panel("plots"):
        ...
        with ui.card().classes("w-full mb-4"):
            with ui.row().classes("gap-4"):
                ...
                ui.button(
                    "Show Archive",
                    on_click=lambda: GUI_functions.create_plots(
                        hours_input, plots_container
                    ),
                ...
                with ui.row().classes("items-center gap-2"):
                    ...
                    status_label = ui.label("Live plots: OFF").classes(
                        "text-sm font-bold"
                    )
        ...
        with plots_container:
            ui.label('Click "Create Plots" first, then "Start Live"').classes(
