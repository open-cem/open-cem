...
from OpenCEM.cem_lib_components import (
    Device,
    PowerSensor,
    HeatPump
)  # TemperatureSensor, RelaisActuator, EVCharger
...
class SwitchingExcessController(Controller):
    ...
    def __init__(
        ...
        *,
        name: str = "",
        mainMeter: PowerSensor,
        deviceMeter: PowerSensor,
        ...
        self.powerLimit = controllerSettings[
            "powerLimit"
        ]  # controller settings: power limit in kW
        self.powerHysteresis = controllerSettings[
            "powerHysteresis"
        ]  # controller settings: power hysteresis in kW
        self.mode = 0  # controller mode (0 = off, 1 = on, 2 = high, etc.)
        self.excess = 0  # actual pv excess in kW

        print(
            f"Controller created: {self.name} type {self.type} "
    ...
    async def calc_controller(self, remainingPower: float):
        ...
        # calculate pv excess
        if remainingPower is None:  # first call
            mainPower, unit, error_code = (
                await self.mainMeter.get_power()
            )  # get main power
            remainingPower = (
                -mainPower
        ...
        if remainingPower > 0:  # remaining power available
            ...
            if self.deviceMeter is not None:  # device meter available
            ...
            remainingPower = (
                remainingPower - ownConsumption
            )  # subtract own consumption from remaining power
        else:
            self.excess = 0
            remainingPower = 0
            ownConsumption = 0
        ...
        print(
            f"Controller calculated: {self.name} type {self.type} "
        ...
        if self.excess > self.powerLimit:
            self.mode = 1
            if old_mode == 0:
                error_code = self.controlledDevice.switch_device(
                    functional_profile=None, state="ON"
                )  # switch device on
        if self.excess < self.powerLimit - self.powerHysteresis:
            self.mode = 0
            if old_mode == 1:
                error_code = self.controlledDevice.switch_device(
                    functional_profile=None, state="OFF"
                )  # switch device off
...
class DynamicExcessController(SwitchingExcessController):
    ...
    async def calc_controller(self, remainingPower: float):
        ...
        error_code, remainingPower = await super().calc_controller(
            remainingPower
        )  # calculate pv excess and switch device (inherited)
        error_code = 0

        if self.excess > self.powerLimit:  # write pv excess as power setpoint to device
            error_code = self.controlledDevice.write_device_setpoint(
                functional_profile="", setpoint=self.excess
            )
...
class TemperatureExcessController(SwitchingExcessController):
    ...
    def __init__(
        ...
        *,
        ...
        mainMeter: PowerSensor,
        deviceMeter: PowerSensor,
        controlledDevice: HeatPump,
        functionalProfile: str = "",
        controllerSettings,
    ):
        super().__init__(
            name=name,
        ...
        self.functionalProfile = (
            functionalProfile  # functional profile (specifies function)
        )
        # controller settings:
        self.tempEco = controllerSettings["tempEco"]  # eco temperature (lowered)
        self.tempComfort = controllerSettings[
            "tempComfort"
        ]  # comfort temperature (normal)
        self.tempMax = controllerSettings["tempMax"]  # maximum temperature (pv excess)
        self.excessComfort = controllerSettings[
            "excessComfort"
        ]  # power limit in kW to switch on tempComfort
        self.excessMax = controllerSettings[
            "excessMax"
        ]  # power limit in kW for tempMax
    ...
    async def calc_controller(self, remainingPower: float):
        ...
        if self.excess > self.excessComfort:
            self.tempSetpoint = (self.excess - self.excessComfort) * (
                self.tempMax - self.tempComfort
            ) / (self.excessMax - self.excessComfort) + self.tempComfort
        ...
        print(
            f"Temperature Controller calculated: {self.name} type {self.type} "
            f"excess {self.excess:.2f} tempSetpoint {self.tempSetpoint:.2f}"
        )

        error_code = self.controlledDevice.write_device_setpoint(
            functional_profile=self.functionalProfile, setpoint=self.tempSetpoint
        )
