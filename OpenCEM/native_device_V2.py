...
class NativeDevice:
    ...
    def replace_placeholders(self, text, placeholder_dict):
    ...
    def update_yaml(self, yaml_data, placeholder_dict):
        """
        ...
        replaced_test = self.replace_placeholders(json_string, placeholder_dict)
        yaml_data = json.loads(replaced_test)
        return yaml_data
    ...
    def connect(self):
        ...
        match com_type:
            ...
            case "MODBUS_RTU":
                #if not modbus_RTU_client:
                #channel zuweisung
                #channel1==>COM2
                #channel2==>COM5
                #in channelliste...channel definieren
                #slave_ID = interface["SlaveAddress"]

                #self.client = AsyncModbusSerialClient(method="rtu", port=port, baudrate=baudrate, parity=parity)
                #modbus_RTU_client = True
                #else
                    #print("RTU client already existing")
                pass
            ...
            case "CLEMAP_CLOUD":
                sensor_id = interface["sensor_id"]
            # cases für clemap, solarmanager, etc
